...
for op in (:+, :-)
    @eval begin
        ...
        @generated function $op{T}(x1::Number, x2::CuArray{T})
            op = $op
            f = CuFunction("""
            __global__ void f($T x1, $T *x2, $T *y, int length) {
                int idx = blockIdx.x * blockDim.x + threadIdx.x;
            ...
            quote
                y = similar(x2)
                $f(T(x1), x2.ptr, y.ptr, length(y), dx=length(y))
                y
            end
        end
        @generated function $op{T}(x1::CuArray{T}, x2::Number)
            op = $op
            f = CuFunction("""
            __global__ void f($T *x1, $T x2, $T *y, int length) {
                int idx = blockIdx.x * blockDim.x + threadIdx.x;
            ...
            quote
                y = similar(x1)
                $f(x1.ptr, x2, y.ptr, length(y), dx=length(y))
                y
            end
        end
    end
