...
function activation(mode, x; relu_nanopt=CUDNN_NOT_PROPAGATE_NAN, relu_ceiling=1.0)
    actdesc = ActivationDesc()
    cudnnSetActivationDescriptor(actdesc, mode, relu_nanopt, relu_ceiling)

    h = handle(x)
    T = eltype(x)
    xdesc = TensorDesc(x)
    y = similar(x)
    cudnnActivationForward(h, actdesc, T[1], xdesc, x, T[0], xdesc, y)

    function backward!(gy, gx)
        isvoid(gx) && return
        cudnnActivationBackward(h, actdesc, T[1], xdesc, y, xdesc, gy, xdesc, x, T[1], xdesc, gx)
    end
    y, backward!
end
